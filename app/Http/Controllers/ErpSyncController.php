...
class ErpSyncController extends Controller
{
    ...
    public function pullCustomers()
    {
        ...
        do {
            ...
            foreach ($batch as $cust) {
                $exists = Customer::where('erp_customer_name', $cust['name'])->first();
                $data   = [
                    'name'              => $cust['customer_name'] ?? $cust['name'],
                    'email'             => $cust['email_id'] ?? null,
                    'phone'             => $cust['mobile_no'] ?? null,
                    'erp_customer_name' => $cust['name'],
                    'erp_last_sync'     => now(),
                    'is_active'         => !($cust['disabled'] ?? false),
                ];
    ...
    public function saveSettings(Request $request)
    {
        ...
        foreach ($request->only(['erpnext_url','erpnext_api_key','erpnext_api_secret','erpnext_company','erpnext_pos_profile']) as $k => $v) {
